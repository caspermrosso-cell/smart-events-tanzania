...
const exportToPDF = (title: string, headers: string[], rows: string[][]) => {
  ...
  printWindow.document.write(`
    ...
    <div class="header">
<img src="/smart-events-logo.png" alt="Smart Events" />
      <div class="address">
...
const Reports = () => {
  ...
  const monthlyPaymentData = useMemo(() => {
    ...
    return months.map(month => {
      ...
      const nbc = monthPayments.filter((p: any) => p.payment_method === 'nbc' || p.payment_method === 'selcom' || p.payment_method === 'mpesa').reduce((s: number, p: any) => s + Number(p.amount), 0);
      ...
      return { name: label, 'NBC': nbc, Bank: bank, Cash: cash, total: nbc + bank + cash };
    });
  }, [payments]);
            ...
            {monthlyPaymentData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={320}>
                <BarChart data={monthlyPaymentData}>
                  ...
                  <Legend />
                  <Bar dataKey="NBC" stackId="a" fill="#22c55e" radius={[0, 0, 0, 0]} />
                    ...
                    <TableRow key={p.id}>
                      ...
                      <TableCell>
                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                          p.payment_method === 'nbc' || p.payment_method === 'selcom' || p.payment_method === 'mpesa' ? 'bg-green-100 text-green-700' :
