...
const Hero = () => {
  ...
  const words = isEn
    ? ['expected.', 'welcomed.', 'remembered.']
    : ['anasubiriwa.', 'anakaribishwa.', 'anakumbukwa.'];
  ...
  const headline = isEn ? ['Every', 'guest', 'arrives'] : ['Kila', 'mgeni', 'afike'];
  ...
  return (
    <section id="hero" className="relative overflow-hidden bg-background pt-28 pb-16 md:pt-36 md:pb-24">
      <motion.div
        ...
        className="pointer-events-none absolute -top-32 -right-24 h-[28rem] w-[28rem] rounded-full bg-accent/25 blur-3xl"
      />
      <motion.div
        aria-hidden
        animate={{ y: [0, -22, 0], opacity: [0.6, 0.95, 0.6] }}
        transition={{ duration: 17, repeat: Infinity, ease: 'easeInOut' }}
        className="pointer-events-none absolute -bottom-40 -left-32 h-[26rem] w-[26rem] rounded-full bg-secondary blur-3xl"
      />
        ...
        >
          ...
          <div className="mb-8 flex flex-col gap-3 sm:flex-row">
            <button
              ...
              className="group inline-flex items-center justify-center gap-2 rounded-full bg-primary px-8 py-4 text-lg font-semibold text-primary-foreground shadow-warm transition-all hover:-translate-y-0.5 hover:bg-foreground"
            >
            ...
            <Link
              to="/how-it-works"
              className="inline-flex items-center justify-center gap-2 rounded-full border border-border bg-card px-8 py-4 text-lg font-semibold text-foreground transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:bg-secondary"
            >
